...
public abstract class FieldHeapKV<V> : HeapKVData<JFieldType, V> {
    ...
    constructor(clz: RefType, fields: PersistentMap<JFieldType, out IHeapValues<V>>, unreferenced: IHeapValues<V>?) : super(fields, unreferenced) {
        this.clz = clz
    }
    ...
    public open fun get(hf: IHeapValuesFactory<Any>, key: JFieldType?): IHeapValues<Any>? {
        return if (key != null) this.getMap()[key] as IHeapValues<Any>? else this.getFromNullKey(hf)
    }
    ...
    public override fun equals(other: Any?): Boolean {
        if (!super.equals(other)) {
            ...
            return other is FieldHeapKV<*> && this.clz == other.clz
        }
    }
