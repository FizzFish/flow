...
public class WObject : SummaryHandlePackage<IValue> {
    public val jClassType: RefType = RefType.v("java.lang.Class")

    public override fun ACheckCallAnalysis.register() {
        this.evalCall(UtilsKt.getSootSignature(TODO("FIXME — unrepresentable INSTANCE") as KCallable<*>), WObject::register$lambda$1)
        this.evalCall(UtilsKt.getSootSignature(TODO("FIXME — unrepresentable INSTANCE") as KCallable<*>), WObject::register$lambda$3)
        this.evalCall(UtilsKt.getSootSignature(TODO("FIXME — unrepresentable INSTANCE") as KCallable<*>), WObject::register$lambda$5)
    }
    ...
    @JvmStatic
    fun register$lambda$1(this$0: WObject, `$this$evalCall`: CalleeCBImpl.EvalCall) {
    ...
    @JvmStatic
    fun CalleeCBImpl.EvalCall.register$lambda$3() {
        val strObjectOp: IOpCalculator = this.getHf().resolveOp(this.getEnv(), this.arg(-1), this.arg(0))
        strObjectOp.resolve(WObject::register$lambda$3$lambda$2)
        this.setReturn(strObjectOp.getRes().build())
    }

    @JvmStatic
    fun register$lambda$5$lambda$4(
        `$this_evalCall`: CalleeCBImpl.EvalCall,
        `$this$resolve`: IOpCalculator,
        res: IHeapValues.Builder,
        var3: Array<CompanionV>
    ...
    @JvmStatic
    fun CalleeCBImpl.EvalCall.register$lambda$5() {
        val unop: IOpCalculator = this.getHf().resolveOp(this.getEnv(), this.arg(0))
        ...
        this.setReturn(unop.getRes().build())
    }
