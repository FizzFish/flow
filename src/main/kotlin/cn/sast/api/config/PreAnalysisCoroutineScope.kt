...
public interface PreAnalysisCoroutineScope : PreAnalysisApi {
   ...
   public abstract fun uninitializedScope() {
   }

   public open suspend fun processPreAnalysisUnits() {
   }
   ...
   internal class DefaultImpls {
      @JvmStatic
      fun <T> runInScene(`$this`: PreAnalysisCoroutineScope, `$context_receiver_0`: CheckerUnit, block: (Continuation<? super T>?) -> Any): Job {
      ...
      fun <P extends ICheckPoint & INodeWithRange> report(
         `$this`: PreAnalysisCoroutineScope, `$receiver`: P, checkType: CheckType, region: Region, env: (BugMessage.Env?) -> Unit
      ) {
         PreAnalysisApi.DefaultImpls.report(`$this`, (P)`$receiver`, checkType, region, env);
      }

      @JvmStatic
      fun report(`$this`: PreAnalysisCoroutineScope, `$receiver`: ISourceFileCheckPoint, checkType: CheckType, region: Region, env: (BugMessage.Env?) -> Unit) {
      ...
      fun report(
         ...
         return PreAnalysisApi.DefaultImpls.report(`$this`, `$receiver`, checkType, cpgRegion, env);
      }
      ...
      fun report(
         ...
         return PreAnalysisApi.DefaultImpls.report(`$this`, `$receiver`, checkType, jpsStart, jpsEnd, env);
      }
      ...
      fun report(
         ...
         return PreAnalysisApi.DefaultImpls.report(`$this`, `$receiver`, checkType, regionNode, env);
      }

      @JvmStatic
      fun report(`$this`: PreAnalysisCoroutineScope, checkType: CheckType, sootHost: Host, region: Region?, env: (BugMessage.Env?) -> Unit) {
